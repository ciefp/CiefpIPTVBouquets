#!/bin/bash
##setup command=wget -q "--no-check-certificate" https://raw.githubusercontent.com/ciefp/CiefpIPTVBouquets/main/installer.sh -O - | /bin/sh
...
TMPPATH=/tmp/CiefpIPTVBouquets

if [ ! -d /usr/lib64 ]; then
	PLUGINPATH=/usr/lib/enigma2/python/Plugins/Extensions/CiefpIPTVBouquets
else
	PLUGINPATH=/usr/lib64/enigma2/python/Plugins/Extensions/CiefpIPTVBouquets
fi
...
if [ -f /var/lib/dpkg/status ]; then
   ...
   wget https://github.com/ciefp/CiefpIPTVBouquets/archive/refs/heads/main.tar.gz
   tar -xzf main.tar.gz
   cp -r 'CiefpIPTVBouquets-main/usr' '/'
set +e
cd
sleep 2
